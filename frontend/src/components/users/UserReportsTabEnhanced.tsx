...
type Report = {
  ...
  behavior_note_type?: 'Positive' | 'Negative' | string;
...
export function UserReportsSection(props: UserReportsProps) {
  const canEdit = props.canEdit ?? true;
  const variant = props.variant ?? 'user';
  const isWorker = variant === 'worker';
  const subjectId = props.variant === 'worker' ? props.workerId : props.userId;
  const reportsPrefix =
  ...
  const fileUploadEmployeeId: string | null = props.variant === 'worker' ? null : props.userId;
  ...
  const hasActiveFilters = Boolean(
    searchQuery || filterType || filterStatus || filterSeverity || filterDateFrom || filterDateTo,
  );
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-6 lg:items-end">
            <AppSelect
              ...
              fieldHint={USER_REPORTS_FIELD_HINTS.type}
            />
            <AppSelect
              label="Status"
              value={filterStatus}
              ...
              fieldHint={USER_REPORTS_FIELD_HINTS.status}
            />
            <AppSelect
              label="Severity"
              ...
              fieldHint={USER_REPORTS_FIELD_HINTS.severity}
            />
            <AppDatePicker
              label="From date"
              value={filterDateFrom}
              onChange={(e) => setFilterDateFrom(e.target.value)}
              fieldHint={USER_REPORTS_FIELD_HINTS.from_date}
            />
            <AppDatePicker
              label="To date"
              value={filterDateTo}
              onChange={(e) => setFilterDateTo(e.target.value)}
              fieldHint={USER_REPORTS_FIELD_HINTS.to_date}
            />
            {hasActiveFilters ? (
...
function CreateReportModal({
  ...
  useEffect(() => {
    ...
    const noteType = report.behavior_note_type;
    setBehaviorNoteType(noteType === 'Positive' || noteType === 'Negative' ? noteType : '');
      ...
      {!report ? (
        ...
        <div className={uiSpacing.sectionStack}>
          ...
          {report.attachments.length > 0 && (
            <AppCard bodyClassName={uiCx(uiSpacing.cardPadding, 'min-w-0')}>
              <AppSectionHeader title="Attachments" description="Files linked to this report." />
              <ul className="mt-3 space-y-2">
                {report.attachments.map((att) => (
                  <li key={att.id} className={uiCx('rounded-lg bg-gray-50 p-2', uiBorders.subtle, 'border')}>
                    <a
                      href={withFileAccessToken(`/files/${att.file_id}`)}
                      target="_blank"
                      rel="noopener noreferrer"
          ...
          <AppCard bodyClassName={uiCx(uiSpacing.cardPadding, 'min-w-0')}>
            <AppSectionHeader
              title={canEdit ? 'History / activities' : 'Comments'}
              description={
                canEdit
                  ? 'Status changes, system events, and user comments on this report.'
                  : 'Comments visible on this report.'
              }
            />
            <div className="mt-3 max-h-64 space-y-3 overflow-y-auto">
              {visibleComments.length === 0 ? (
                ...
                visibleComments.map((comment) => (
                  <div
                    key={comment.id}
                    className={uiCx('rounded-lg border-l-4 border-gray-300 bg-gray-50 p-3', uiBorders.subtle)}
                  >
                    <div className={uiCx(uiTypography.helper, 'mb-1 flex flex-wrap items-center gap-2')}>
                      <span>
                    ...
                    <div className={uiCx(uiTypography.body, 'text-gray-700')}>{comment.comment_text}</div>
                  </div>
                ))
              )}
            </div>
