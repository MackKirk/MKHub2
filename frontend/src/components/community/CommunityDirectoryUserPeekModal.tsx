  ...
  return (
    <div className="flex gap-3 py-2.5 border-b border-slate-100 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div
          className={`text-sm break-words ${muted ? 'font-normal text-slate-400' : 'font-medium text-slate-900'}`}
        >
          {value}
        </div>
      </div>
...
export default function CommunityDirectoryUserPeekModal({ userId, onClose }: Props) {
  ...
  return (
    <OverlayPortal>
      <div
        className="fixed inset-0 z-[200010] flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-[2px]"
        role="presentation"
        onClick={onClose}
      >
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="directory-user-peek-title"
        ...
        >
          <div className="relative flex h-[5.75rem] shrink-0 items-center justify-center overflow-hidden border-b border-slate-800/40 px-5 sm:px-8">
            <div
              className="absolute inset-0 bg-gradient-to-b from-slate-600 via-slate-700 to-slate-800"
              aria-hidden
            />
            <div
