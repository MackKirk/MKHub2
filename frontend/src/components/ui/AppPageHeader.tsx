...
export function AppPageHeader({ title, subtitle, icon, actions, className }: AppPageHeaderProps) {
  return (
    <header className={uiCx(uiLayout.contentContainer, className)}>
      <div className={uiCx('flex flex-wrap items-start justify-between gap-3', uiRadius.card, uiBorders.subtle, uiColors.surface, uiSpacing.cardPadding)}>
        <div className="flex min-w-0 items-start gap-3">
          {icon ? <div className={uiCx('mt-0.5 flex h-8 w-8 items-center justify-center bg-blue-100 text-blue-800', uiRadius.control)}>{icon}</div> : null}
