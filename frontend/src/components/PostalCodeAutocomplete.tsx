...
function extractPostalFromParsed(parsed: ReturnType<typeof parseGooglePlaceResult>): string {
  if (parsed.postal_code) return parsed.postal_code;
  const m = (parsed.address_line1 || '').match(/\b([A-Z0-9]{2,}\s?[A-Z0-9]{2,})\b/i);
  ...
  const fetchPredictions = useCallback(async (text: string) => {
    ...
    try {
      const d: { predictions?: Prediction[] } = await api(
        'GET',
        `/integrations/places/autocomplete?q=${encodeURIComponent(t)}&types=geocode`
      );
  ...
  const selectPrediction = useCallback(
    async (p: Prediction) => {
      ...
      try {
        ...
        const postal = extractPostalFromParsed(parsed);
        if (!postal) return;
        onChange(postal);
        onPostalCodeSelect?.({
          postal_code: postal,
