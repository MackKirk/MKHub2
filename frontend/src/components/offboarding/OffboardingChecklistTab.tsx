...
export default function OffboardingChecklistTab({
  ...
  if (isLoading) return <div className="text-sm text-gray-500 p-4">Loading checklist…</div>;
  if (!items.length) return <AppEmptyState title="Checklist not available yet" />;

  return (
    <div className="space-y-2">
      {items.map((item) => (
        <AppCard key={item.item_key} className="p-3 flex flex-wrap items-center justify-between gap-3">
