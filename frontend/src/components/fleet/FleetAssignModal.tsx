...
const FORM_ID = 'fleet-assign-form';

const FLEET_ASSIGN_QUICK_INFO = formModalQuickInfo({
  purpose: <>Assign this fleet asset to a team member and record check-out details for the assignment log.</>,
  howToUse: (
    <>
      Choose {uiLabel('Name')}, then confirm or edit phone, address, sleep location, and department. Add odometer or
      hours out, photos, and notes before assigning.
    </>
  ),
  actions: (
    <>
...
export default function FleetAssignModal({
  open,
  asset,
  assetDisplayName,
  employees,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (asset?.asset_type === 'vehicle' && odometer_out.trim() !== '' && minOdometerOut != null) {
      ...
      if (!Number.isNaN(out) && out < minOdometerOut) {
        toast.error(`Odometer out must be at least the current reading (${minOdometerOut.toLocaleString()}).`);
        return;
      }
    }
    ...
    onSubmit({
      ...
      odometer_out:
        asset?.asset_type === 'vehicle' && odometer_out
          ? parseInt(odometer_out, 10)
          : odometer_out
            ? parseInt(odometer_out, 10)
            : null,
      hours_out:
        (asset?.asset_type === 'heavy_machinery' || asset?.asset_type === 'other') && hours_out
          ? parseFloat(hours_out)
          : hours_out
            ? parseFloat(hours_out)
            : null,
  ...
  const title = assetDisplayName?.trim() ? `Assign — ${assetDisplayName.trim()}` : 'Assign';
  ...
  return (
    <AppFormModal
      ...
      onClose={onClose}
      title={title}
      description="Assign this fleet asset to a team member. Checkout details and photos are saved on the assignment log."
      formWidth="comfortable"
      quickInfo={FLEET_ASSIGN_QUICK_INFO}
      footer={
    ...
    >
      <form id={FORM_ID} onSubmit={handleSubmit} className={uiSpacing.sectionStack}>
        <div className={uiLayout.sectionGrid2}>
          ...
          <div className="min-w-0 space-y-1.5 md:col-span-2">
            <AppControlLabelRow label="Sleep" fieldHint={<AppFieldHint hint={H.sleeps_snapshot} />} />
            <AddressAutocomplete
              value={sleeps_snapshot}
              onChange={setSleepsSnapshot}
              placeholder="Sleep / yard location"
              disabled={busy}
              className={controlInputClass}
            />
          </div>
          ...
          {(asset?.asset_type === 'heavy_machinery' || asset?.asset_type === 'other') && (
            <AppInput
              ...
              step="0.1"
              value={hours_out}
              onChange={(e) => setHoursOut(e.target.value)}
              min={0}
              disabled={busy}
              fieldHint={H.hours_out}
            />
          )}
        </div>
