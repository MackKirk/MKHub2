...
import { AppCard, AppSectionHeader, appSectionPresetProps, uiSpacing } from '@/components/ui';
...
export function SubcontractorCompanyFilesTabEnhanced({
  ...
  const handleMoveFile = async (fileId: string, newCategory: string) => {
    try {
      if (!canEditFiles) return;
      await api('PUT', `/subcontractors/companies/${companyId}/files/${fileId}`, { category: newCategory === 'uncategorized' ? null : newCategory });
      await onRefresh();
