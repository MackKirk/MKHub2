...
type Bidding = { id:string, code?:string, name?:string, slug?:string, client_id?:string, created_at?:string, date_start?:string, date_end?:string, is_bidding?:boolean };
...
export default function Biddings(){
  ...
  const { data, isLoading, refetch } = useQuery({ queryKey:['biddings', qs], queryFn: ()=>api<Bidding[]>('GET', `/projects?is_bidding=true${qs}`) });
  const arr = data||[];
  ...
  return (
    <div>
      <div className="mb-3 rounded-xl border bg-gradient-to-br from-[#7f1010] to-[#a31414] text-white p-4">
        <div className="text-2xl font-extrabold">Biddings</div>
      ...
      <div className="mb-3 rounded-xl border bg-white p-3 flex items-end gap-2">
        ...
        <Link to="/projects/new?is_bidding=true" state={{ backgroundLocation: location }} className="px-3 py-2 rounded bg-black text-white">New Bidding</Link>
      </div>
      <div className="grid md:grid-cols-4 gap-2">
        {isLoading? <div className="h-32 bg-gray-100 animate-pulse rounded"/> : arr.map(p => (
          <BiddingListCard key={p.id} bidding={p} />
        ))}
      </div>
      {pickerOpen?.open && (
...
function BiddingListCard({ bidding }:{ bidding: Bidding }){
  const { data:files } = useQuery({ queryKey:['client-files-for-bidding-card', bidding.client_id], queryFn: ()=> bidding.client_id? api<any[]>('GET', `/clients/${encodeURIComponent(String(bidding.client_id))}/files`) : Promise.resolve([]), enabled: !!bidding.client_id, staleTime: 60_000 });
  const pfiles = useMemo(()=> (files||[]).filter((f:any)=> String((f as any).project_id||'')===String(bidding.id)), [files, bidding?.id]);
  ...
  const { data:details } = useQuery({ queryKey:['bidding-detail-card', bidding.id], queryFn: ()=> api<any>('GET', `/projects/${encodeURIComponent(String(bidding.id))}`), staleTime: 60_000 });
  const { data:client } = useQuery({ queryKey:['bidding-client', bidding.client_id], queryFn: ()=> bidding.client_id? api<any>('GET', `/clients/${encodeURIComponent(String(bidding.client_id||''))}`): Promise.resolve(null), enabled: !!bidding.client_id, staleTime: 300_000 });
  const status = (bidding as any).status_label || details?.status_label || '';
  const progress = Math.max(0, Math.min(100, Number((bidding as any).progress ?? details?.progress ?? 0)));
  const start = (bidding.date_start || details?.date_start || bidding.created_at || '').slice(0,10);
  const clientName = client?.display_name || client?.name || '';
  return (
    <Link to={`/biddings/${encodeURIComponent(String(bidding.id))}`} className="group rounded-lg border overflow-hidden bg-white block">
      <div className="aspect-[4/3] bg-gray-100 relative">
        <img className="w-full h-full object-cover" src={src} />
        <button onClick={(e)=>{ e.preventDefault(); e.stopPropagation(); }} className="absolute right-2 top-2 text-xs px-2 py-1 rounded bg-black/70 text-white" title="Change cover (open bidding)">Cover</button>
      </div>
      <div className="p-2">
        <div className="text-xs text-gray-600 truncate">{clientName||''}</div>
        <div className="font-semibold text-sm truncate group-hover:underline">{bidding.name||'Bidding'}</div>
        <div className="text-xs text-gray-600 truncate">{bidding.code||''}</div>
