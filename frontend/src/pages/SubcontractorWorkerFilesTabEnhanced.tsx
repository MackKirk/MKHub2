...
import { AppCard, AppSectionHeader, appSectionPresetProps } from '@/components/ui';
...
export function SubcontractorWorkerFilesTabEnhanced({
  ...
  const handleMoveFile = async (fileId: string, newCategory: string) => {
    try {
      if (!canEditFiles) return;
      await api('PUT', `/subcontractors/workers/${workerId}/files/${fileId}`, { category: newCategory === 'uncategorized' ? null : newCategory });
      await onRefresh();
