...
import { AppButton, AppCard, AppSectionHeader, appSectionPresetProps, AppCheckboxControl, AppListRowIconButton, uiCx } from '@/components/ui';
...
export function SubcontractorWorkerFilesTabEnhanced({
  workerId,
  files,
  onRefresh,
  hasEditPermission,
  ...
  /** Admin-only: Library vs soft-deleted worker files. */
  const [filesSection, setFilesSection] = useState<'active' | 'deleted'>('active');
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const { viewMode, tileSize, setViewMode, setTileSize } = usePersistedFileViewMode('subcontractor-worker-files-view', {
    category:
  ...
  const { data: deletedFiles = [], refetch: refetchDeletedFiles } = useQuery({
    queryKey: ['subcontractorWorkerDeletedFiles', workerId],
    queryFn: () => api<ClientDeletedFile[]>('GET', `/subcontractors/workers/${encodeURIComponent(workerId)}/files/deleted`),
    enabled: !!workerId && isAdmin && filesSection === 'deleted',
  });
  ...
  const moveFilesToCategory = async (fileIds: string[], newCategory: string, label?: string) => {
    ...
    const filesQueryKey = ['subcontractor-worker-files', workerId] as const;
    ...
    const results = await Promise.allSettled(
      unique.map((fileId) =>
        api('PUT', `/subcontractors/workers/${workerId}/files/${fileId}`, { category: categoryValue }),
      ),
    );
    ...
    if (ok === 0) {
      ...
      invalidateQueriesInBackground(queryClient, [
        filesQueryKey,
        ['subcontractor-worker', workerId],
        ['subcontractor-worker-activity', workerId],
      ]);
    }
    if (ok === unique.length) {
  ...
  const handleBulkDownloadSelected = async () => {
    ...
    try {
      await downloadStoredFiles(payload, 'subcontractor-files.zip');
  ...
  const handleBulkDeleteSelected = async () => {
    ...
    const result = await confirm({
      ...
      message: `Remove ${ids.length} file(s) from the worker library?`,
      confirmText: 'Delete',
      cancelText: 'Cancel',
    });
    ...
    try {
      const results = await Promise.allSettled(
        ids.map((fileId) => api('DELETE', `/subcontractors/workers/${workerId}/files/${fileId}`)),
      );
      const ok = results.filter((r) => r.status === 'fulfilled').length;
      await queryClient.invalidateQueries({ queryKey: ['subcontractorWorkerDeletedFiles', workerId] });
      await onRefresh();
      fileSelection.clear();
  ...
  const uploadMultiple = async (fileList: File[], targetCategory?: string) => {
    ...
    for (const item of newQueue) {
      try {
        ...
        await api(
          'POST',
          `/subcontractors/workers/${workerId}/files?file_object_id=${encodeURIComponent(conf.id)}&category=${encodeURIComponent(category || '')}&original_name=${encodeURIComponent(item.file.name)}`
        );
  ...
  const handleDeleteFile = async (fileId: string) => {
    ...
    try {
      if (!canEditFiles) return;
      await api('DELETE', `/subcontractors/workers/${workerId}/files/${fileId}`);
      await queryClient.invalidateQueries({ queryKey: ['subcontractorWorkerDeletedFiles', workerId] });
      await onRefresh();
  ...
  const handlePermanentDeleteFile = async (fileId: string) => {
    ...
    if (result !== 'confirm') return;
    try {
      await api('DELETE', `/subcontractors/workers/${encodeURIComponent(workerId)}/files/deleted/${encodeURIComponent(fileId)}`);
  ...
  const handleRestoreDeletedFile = async (fileId: string) => {
    try {
      await api('POST', `/subcontractors/workers/${encodeURIComponent(workerId)}/files/deleted/${encodeURIComponent(fileId)}/restore`);
      await refetchDeletedFiles();
      await onRefresh();
  ...
  const handleRenameFile = async (fileId: string, newName: string) => {
    ...
    if (!canEditFiles) return;
    try {
      await api('PUT', `/subcontractors/workers/${workerId}/files/${fileId}`, { original_name: trimmed });
  ...
  return (
    <div className="space-y-4">
      <AppCard>
        <AppSectionHeader
          title="Documents"
          description="Insurance, IDs, certifications, and other files for this worker."
        ...
        {isAdmin && filesSection === 'deleted' ? (
          <div className="rounded-xl border border-amber-100 bg-amber-50/50 overflow-hidden">
            <p className="text-xs text-amber-900 px-3 py-2 border-b border-amber-100/80">
              Same previews and downloads as the library. Restore returns the file to the active worker file list, or delete permanently to remove it from storage.
            </p>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {Array.isArray(deletedFiles) && deletedFiles.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-16 text-gray-500 text-sm">
                    ...
                    <div>No deleted files for this worker.</div>
                  </div>
                )}
              </div>
            </div>
          ...
          <ProjectFilesHome
            title="Subcontractor Worker Files"
            description="Browse files by category or open the complete worker library."
        ...
        <ImageEditor
          ...
          onSave={async (blob) => {
            try {
              ...
              await api(
                'POST',
                `/subcontractors/workers/${encodeURIComponent(workerId)}/files?file_object_id=${encodeURIComponent(conf.id)}&category=${encodeURIComponent(originalFile?.category || 'image-edited')}&original_name=${encodeURIComponent(editedName)}`
              );
