/** Shared fleet inspection checklist form helpers (aligned with web fleetInspectionFormShared.ts). */
...
export function isValidInspectionCondition(
  condition: string
): condition is InspectionConditionValue {
...
export function buildBodyFormFromInspection(
  ...
  const metadata =
    cr?._metadata && typeof cr._metadata === "object"
      ? { ...(cr._metadata as Record<string, string>) }
      : {};
  if (asset) {
    const hasUnit = metadata.unit_number != null && String(metadata.unit_number).trim() !== "";
    if (!hasUnit && asset.unit_number != null && String(asset.unit_number).trim() !== "") {
      metadata.unit_number = asset.unit_number;
    }
    if (!hasUnit && asset.name != null && String(asset.name).trim() !== "") {
      metadata.unit_number = (metadata.unit_number as string) || asset.name;
    }
    if (
      (metadata.km == null || String(metadata.km).trim() === "") &&
      asset.odometer_current != null
    ) {
      metadata.km = String(asset.odometer_current);
    }
  }
  ...
  const areas = (templateAreas || []).map((area) => {
    const areasArr =
      (cr?.areas as Array<{ key: string; condition?: string }> | undefined) ?? [];
    ...
    section.items.forEach((item) => {
      ...
      const condition =
        typeof val === "object" && val != null
          ? (val as { status?: string; condition?: string }).status ||
            (val as { condition?: string }).condition ||
            ""
          : (val as string) || "";
      const norm =
        condition === "ok" || condition === "damage" || condition === "conditional"
          ? condition
          : "";
...
export function buildBodyChecklistPayload(
  form: BodyFormState,
  fleetAsset?: FleetAssetFormContext
): Record<string, unknown> {
  const _metadata: Record<string, string> = {};
  if (fleetAsset) {
    if (fleetAsset.unit_number || fleetAsset.name) {
      _metadata.unit_number = fleetAsset.unit_number || fleetAsset.name || "";
    }
    if (fleetAsset.odometer_current != null) {
      _metadata.km = String(fleetAsset.odometer_current);
    }
...
export function buildMechanicalChecklistPayload(
  form: MechanicalFormState,
  fleetAsset?: FleetAssetFormContext
): Record<string, unknown> {
  const _metadata: Record<string, string> = {};
  if (fleetAsset) {
    ...
    if (fleetAsset.hours_current != null) {
      _metadata.hours = String(fleetAsset.hours_current);
    }
    _metadata.date = new Date().toISOString().slice(0, 10);
  }
  return {
...
export function isInspectionResultFinal(result?: string | null): boolean {
  if (!result) return false;
  return ["pass", "fail", "conditional"].includes(String(result).toLowerCase());
}

export function inspectionServerSyncKey(inspection: {
  id: string;
  result: string;
