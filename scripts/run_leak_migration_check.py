"""Validate leak investigation division placement (subdivision under Roof Assessments)."""
...
DIV_SQL = text(
    ...
    JOIN setting_items parent ON parent.list_id = sl.id
      AND parent.label = 'Roof Assessments'
...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        if top_level_leak and int(top_level_leak) > 0:
            print(
                "FAIL: Leak Investigations still top-level — run migrate_rm_project_divisions_2026_v2.py",
                file=sys.stderr,
            )
            sys.exit(1)
        if pm_top_level and int(pm_top_level) > 0:
            print(
                "FAIL: Preventive Maintenance still top-level — run migrate_rm_project_divisions_2026_v2.py",
                file=sys.stderr,
            )
            sys.exit(1)
        if cs_leak and int(cs_leak) > 0:
            print(
                "FAIL: Leak Investigations still under Commercial Service — run migrate_rm_project_divisions_2026_v2.py",
                file=sys.stderr,
            )
            sys.exit(1)
        if not leak_div_id:
            print("FAIL: Leak Investigations subdivision not found under Roof Assessments", file=sys.stderr)
            sys.exit(1)
        if flags_after != 0:
