...
async function loadDraft(){
  ...
  try{
    ...
    (d.sections||[]).forEach(sec=>{ addSectionBtn.click(); const last = sectionsContainer.querySelector('.sectionItem:last-child'); last.querySelector(`[name^="section_title_"]`).value = sec.title||''; last.querySelector('[data-role="section-type"]').value = sec.type||'text'; renderSectionContent(last, sec.type||'text'); if (sec.type==='text'){ last.querySelector(`[name^="section_text_"]`).value = sec.text||''; } else { (sec.images||[]).forEach(im=>{ const imagesBox = last.querySelector('[data-role="images-box"]'); const wrap=document.createElement('div'); wrap.className='imageItem'; if (im && im.file_object_id){ wrap.dataset.fileId = im.file_object_id; wrap.innerHTML = `<div class="muted">From site: ${im.file_object_id}</div><label>Caption:<input type="text" maxlength="90"></label><div class="inline-actions"><button type="button" class="btn-danger" data-action="remove-image">Remove image</button></div>`; } else { wrap.innerHTML = `<label>Upload Image:<input type=\"file\" accept=\"image/*\"></label><label>Caption:<input type=\"text\" maxlength=\"90\"></label><div class=\"inline-actions\"><button type=\"button\" class=\"btn-danger\" data-action=\"remove-image\">Remove image</button></div>`; } imagesBox.appendChild(wrap); const input = wrap.querySelector('input[type="text"]'); if (input) input.value = (im && im.caption)||''; }); } });
    calculateTotal();
  }catch(e){}
}
loadDraft();
