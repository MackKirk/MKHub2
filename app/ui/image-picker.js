...
(function(){
  ...
  async function open(opts){
    ...
    return new Promise(async (resolve) => {
      ...
      function drawOverlay(){ const ctx=overlay.getContext('2d'); ctx.clearRect(0,0,overlay.width, overlay.height); for (const it of ES.items){ ctx.save(); ctx.strokeStyle=it.color; ctx.fillStyle=it.color; ctx.lineWidth=it.stroke; if (it.type==='rect'){ ctx.strokeRect(it.x,it.y,it.w,it.h); } else if (it.type==='arrow'){ const dx=it.x2-it.x, dy=it.y2-it.y; const len=Math.hypot(dx,dy)||1; const ux=dx/len, uy=dy/len; const head=10+it.stroke*2; ctx.beginPath(); ctx.moveTo(it.x,it.y); ctx.lineTo(it.x2,it.y2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(it.x2,it.y2); ctx.lineTo(it.x2-ux*head-uy*head*0.5, it.y2-uy*head+ux*head*0.5); ctx.lineTo(it.x2-ux*head+uy*head*0.5, it.y2-uy*head-ux*head*0.5); ctx.closePath(); ctx.fill(); } else if (it.type==='text'){ ctx.font=it.font; ctx.fillText(it.text||'', it.x, it.y); } if (ES.selectedId===it.id){ ctx.setLineDash([4,3]); ctx.strokeStyle='#3b82f6'; const bb=itemBounds(it); if (bb){ ctx.strokeRect(bb.x,bb.y,bb.w,bb.h); } } ctx.restore(); }
      }
      ...
      let dragging=false, startX=0, startY=0; cvs.addEventListener('mousedown', (e)=>{ if (ES.mode!=='pan') return; dragging=true; startX=e.offsetX; startY=e.offsetY; }); cvs.addEventListener('mousemove', (e)=>{ if (!dragging||ES.mode!=='pan') return; ES.offsetX+=(e.offsetX-startX); ES.offsetY+=(e.offsetY-startY); startX=e.offsetX; startY=e.offsetY; redraw(); }); window.addEventListener('mouseup', ()=>{ dragging=false; });
      let drawing=null; overlay.style.pointerEvents='auto'; overlay.addEventListener('mousedown', (e)=>{ const x=e.offsetX, y=e.offsetY; if (ES.mode==='rect'){ drawing={ id:'it_'+Date.now(), type:'rect', x, y, w:1, h:1, color:ES.color, stroke:ES.stroke }; ES.items.push(drawing); ES.selectedId=drawing.id; redraw(); } else if (ES.mode==='arrow'){ drawing={ id:'it_'+Date.now(), type:'arrow', x, y, x2:x+1, y2:y+1, color:ES.color, stroke:ES.stroke }; ES.items.push(drawing); ES.selectedId=drawing.id; redraw(); } else if (ES.mode==='text'){ const it={ id:'it_'+Date.now(), type:'text', x, y, text:ES.text, font:ES.font, color:ES.color, stroke:ES.stroke }; ES.items.push(it); ES.selectedId=it.id; redraw(); } else { const hit=itemAt(e.offsetX,e.offsetY); ES.selectedId = hit ? hit.id : null; redraw(); } });
      overlay.addEventListener('mousemove', (e)=>{ if (!drawing) return; if (drawing.type==='rect'){ drawing.w=(e.offsetX-drawing.x); drawing.h=(e.offsetY-drawing.y); } if (drawing.type==='arrow'){ drawing.x2=e.offsetX; drawing.y2=e.offsetY; } redraw(); }); window.addEventListener('mouseup', ()=>{ drawing=null; });
