        ...
        try:
            ...
            try:
                ...
                if dialect != "postgresql" and "postgresql" not in dialect:
                    ...
                    try:
                        insp = inspect(engine)
                        if insp.has_table("employee_training_records"):
                            cols = insp.get_columns("employee_training_records")
                            cd = next((c for c in cols if c["name"] == "completion_date"), None)
                            if cd is not None and cd.get("nullable") is False:
                                if engine.dialect.name == "postgresql":
                                    db.execute(
                                        text(
                                            "ALTER TABLE employee_training_records ALTER COLUMN completion_date DROP NOT NULL"
                                        )
                                    )
                                    db.commit()
                                    print("[startup] employee_training_records.completion_date is now nullable")
                                    ...
                                    try:
                                        ...
                                        db.commit()
                                        print("[startup] employee_training_records.completion_date is now nullable (sqlite)")
                                    except Exception as _e:
                                        print(
                                            f"[startup] SQLite: could not relax completion_date NOT NULL ({_e}); delete dev.db or migrate manually"
                                        )
                    except Exception as _mig_e:
                        print(f"[startup] employee_training_records completion_date migration (non-critical): {_mig_e}")

                    try:
                        insp2 = inspect(engine)
                        if insp2.has_table("employee_training_records"):
                            have = {c["name"] for c in insp2.get_columns("employee_training_records")}
                            pg = engine.dialect.name == "postgresql"
                            for col_name, ddl_pg, ddl_sqlite in (
                                ("crew", "VARCHAR(200)", "TEXT"),
                                ("location", "VARCHAR(500)", "TEXT"),
                                ("session_time", "VARCHAR(120)", "TEXT"),
                            ):
                                if col_name in have:
                                    continue
                                ...
                                try:
                                    db.execute(
                                        text(f"ALTER TABLE employee_training_records ADD COLUMN {col_name} {ddl}")
                                    )
                                    db.commit()
                                    print(f"[startup] Added employee_training_records.{col_name}")
                                except Exception as _ce:
                                    db.rollback()
                                    print(f"[startup] employee_training_records.{col_name} add skipped: {_ce}")
                    except Exception as _et_e:
                        print(f"[startup] employee_training_records schedule columns (non-critical): {_et_e}")
